...
The sweep workflow runs:
    ...
    record    → THIS SCRIPT runs in the record job:
                joins each matrix row with its downloaded artifact and
                emits a JSON array of envelopes suitable for
                append_history.py.

A row is recorded as `pass` only when both build_outcome and test_outcome
are "success". Anything else (failure, cancelled, skipped, missing
artifact) is `fail`. Missing artifacts surface a ::warning and use a
40-char zero sentinel for resolved_sha.
...
def find_result(
    ...
    """
    artifact_dir = results_dir / f"validate-result-{distro}-{package}-{autoware_version}"
...
def envelope_for(row: dict, result: dict | None, sweep_kind: str, at: str, run_url: str) -> dict:
    distro = row["ros_distro"]
    package = row["package_name"]
    autoware_version = row["autoware_version"]
...
def main() -> None:
    ...
    for row in rows:
        result = find_result(
            ...
            row["ros_distro"],
            row["package_name"],
            row["autoware_version"],
        )
        if result is None:
            print(
                f"::warning::no result artifact for "
                f"{row['ros_distro']}/{row['package_name']}@{row['autoware_version']}",
                file=sys.stderr,
            )
