...
<style>
  :root {{ --bg:#ffffff; --panel:#f7f8fb; --border:#dbe0ea; --fg:#1c2333; --muted:#6b7280;
           --primary:#7290E5; --secondary:#EED45B; --pass:#2ea043; --fail:#d1242f; --unknown:#9aa1ab; }}
  * {{ box-sizing:border-box; }}
  body {{ margin:0; background:var(--bg); color:var(--fg);
          border-top:4px solid; border-image:linear-gradient(90deg,var(--primary),var(--secondary)) 1;
  ...
  .card {{ background:var(--panel); border:1px solid var(--border); border-radius:12px;
           padding:18px 20px; margin-bottom:14px; box-shadow:0 1px 2px rgba(27,35,51,.05); }}
  ...
  .badge-distro {{ border-color:var(--primary); color:var(--primary); }}
  ...
  .tag {{ font-size:12px; background:#eef1f8; border:1px solid var(--border); border-radius:6px; padding:1px 8px; color:#4a5260; }}
  .meta {{ font-size:13px; }}
  .repo {{ color:var(--primary); text-decoration:none; word-break:break-all; }}
  .maintainers {{ font-size:13px; color:var(--muted); margin-top:6px; }}
  .maintainers a, .versions a {{ color:var(--primary); text-decoration:none; }}
...
<header class="site">
  ...
  <p>Federated registry of community Autoware packages. Each package is validated against the
     ...
     on the <a href="https://github.com/autowarefoundation/autoware-index/tree/data" style="color:var(--primary)">data branch</a>.</p>
  <div class="stats">
